...
pmx.initModule({
    ...
    const sendWorkerRequest = (id, requestId) => {
    ...
    const requestHandler = (req, res) => {
        ...
        if (query.pm_id) {
            ...
            sendWorkerRequest(Number(query.pm_id), requestId);
        } else {
            pm2.list((err, apps) => {
                if (err) {
                    return res.end(err.message);
                ...
                if (workers.length === 0) {
                    return setImmediate(() => done(null, "no metrics"));
                }
                ...
                workers.forEach(worker => {
                    sendWorkerRequest(worker.pm_id, requestId);
                });
            });
        }
    }
        ...
        bus.on(GET_METRICS_RES, message => {
            ...
            if (request.pending === 0) {
                ...
                try {
                    ...
                    request.done("aggregate error prom-client version require >= 11.0.0:", err);
                }
            }
        });
    });
